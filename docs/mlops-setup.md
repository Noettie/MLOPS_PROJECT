...
- You must be a Databricks workspace admin in the staging and prod workspaces. Verify that you're an admin by viewing the
  [staging workspace admin console](https://adb-2396132839216422.2.azuredatabricks.net#setting/accounts) and
  [prod workspace admin console](https://adb-6507536166628980.0.azuredatabricks.net#setting/accounts). If
...
1. Create a new variable group called `ml-devops-pipeline variable group` defining the following secret variables, for more details [here](https://learn.microsoft.com/en-us/azure/devops/pipelines/library/variable-groups?view=azure-devops&tabs=classic#create-a-variable-group):
